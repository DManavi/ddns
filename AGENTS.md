...
The release and publish workflows are gated on a `RELEASE_ENABLED` repository
variable — every job in `release.yml`, `publish-ghcr.yml`,
`publish-dockerhub.yml` and `publish-packagist.yml` skips when it isn't
`true`. `release.yml` rewrites `Bootstrap::VERSION` before tagging — so if you
rename that constant, it has to change with it. CI checks the constant is
still in the shape the release rewrites, rather than letting a release be
where that is discovered. Publishing to ghcr.io, Docker Hub and Packagist
happens in the other three workflows, each its own run started explicitly by
`release.yml` after it creates the release (see that workflow's last step for
why it cannot just rely on their `release: published` trigger) — ghcr.io and
Docker Hub are two of those rather than one workflow pushing to both, so a
Docker Hub credential problem cannot also stop ghcr.io, which needs no
